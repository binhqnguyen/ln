...
static int ipv6_destopt_rcv(struct sk_buff *skb)
{
 ...
 if (!pskb_may_pull(skb, skb_transport_offset(skb) + 8) ||
  ...
  ({ struct inet6_dev *_idev = (ip6_dst_idev(skb->dst)); if (__builtin_expect(!!(_idev != ((void *)0)), 1))
 (({ (void)((0)); (((_idev)->stats.ipv6[0])); })->mibs[(IPSTATS_MIB_INHDRERRORS)]++); (({ (void)((0)); (((*_GLOBAL_ipv6_statistics_20_A[get_stack_id()])[0])); })->mibs[(IPSTATS_MIB_INHDRERRORS)]++); });
  kfree_skb(skb);
  return -1;
 }
...
static int ipv6_rthdr_rcv(struct sk_buff *skb)
{
 ...
 if (ipv6_addr_is_multicast(&ipv6_hdr(skb)->daddr) ||
     skb->pkt_type != 0) {
  ({ struct inet6_dev *_idev = (ip6_dst_idev(skb->dst)); if (__builtin_expect(!!(_idev != ((void *)0)), 1))
 (({ (void)((0)); (((_idev)->stats.ipv6[0])); })->mibs[(IPSTATS_MIB_INADDRERRORS)]++); (({ (void)((0)); (((*_GLOBAL_ipv6_statistics_20_A[get_stack_id()])[0])); })->mibs[(IPSTATS_MIB_INADDRERRORS)]++); });
  kfree_skb(skb);
  return -1;
 }
...
# 394 "linux-2.6.26/net/ipv6/exthdrs.c"
 ...
 if (skb_cloned(skb)) {
  ...
  if (pskb_expand_head(skb, 0, 0, ((( gfp_t)0x20u)))) {
   ({ struct inet6_dev *_idev = (ip6_dst_idev(skb->dst)); if (__builtin_expect(!!(_idev != ((void *)0)), 1))
 (({ (void)((0)); (((_idev)->stats.ipv6[0])); })->mibs[(IPSTATS_MIB_OUTDISCARDS)]++); (({ (void)((0)); (((*_GLOBAL_ipv6_statistics_20_A[get_stack_id()])[0])); })->mibs[(IPSTATS_MIB_OUTDISCARDS)]++); });
   kfree_skb(skb);
   return -1;
  }
  hdr = (struct ipv6_rt_hdr *)skb_transport_header(skb);
...
static int ipv6_hop_jumbo(struct sk_buff *skb, int optoff)
{
 ...
 if (nh[optoff + 1] != 4 || (optoff & 3) != 2) {
  do { if (global_net_msg_warn[get_stack_id()] && net_ratelimit()) printk("<7>" "ipv6_hop_jumbo: wrong jumbo opt length/alignment %d\n", nh[optoff+1]); } while(0)
                       ;
  ({ struct inet6_dev *_idev = (ipv6_skb_idev(skb)); if (__builtin_expect(!!(_idev != ((void *)0)), 1))
 (({ (void)((0)); (((_idev)->stats.ipv6[0])); })->mibs[(IPSTATS_MIB_INHDRERRORS)]++); (({ (void)((0)); (((*_GLOBAL_ipv6_statistics_20_A[get_stack_id()])[0])); })->mibs[(IPSTATS_MIB_INHDRERRORS)]++); });
  goto drop;
 }
