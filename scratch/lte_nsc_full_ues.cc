...
std::string TCP_MAX_TX_BUFFER="131071"; //between 64KB and 4MB, default 131071B
std::string TCP_MAX_RX_BUFFER="390000";  //between 87380B and 6MB, default 131071B
uint16_t isAMRLC = 0;    
...
//		LogComponentEnable("PacketSink",LOG_LEVEL_INFO);
   ...
   // LogComponentEnable("TcpSocketBase",level);
    ...
    LogComponentEnable ("NscTcpSocketImpl",level);
...
init_wrappers(){
    ...
    //********************Initialize wrappers*********************/
    if (isTcp==1){
      put_send = DIR + "tcp-put-all.dat";
      put_ack = DIR + "tcp-put-all-ack.dat";
      macro = DIR + "macro_tcp.dat";
    } else{
      put_send = DIR + "udp-put-all.dat";
